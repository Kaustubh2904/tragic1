...
class ImageParser:
    ...
    def parse(self) -> Tuple[List[Obstacle], List[Exit]]:
        ...
        # Red detection (flexible - any reddish hue)
        red_mask = (red_channel > green_channel + 20) & (red_channel > blue_channel + 20)
        ...
        if np.any(red_mask):
            ...
            self._extract_exits_from_mask(red_mask)
        else:
    ...
    def _extract_obstacles_from_mask(self, mask: np.ndarray):
        ...
        for y in range(0, height, cell_size):
            for x in range(0, width, cell_size):
                ...
                if cell.size > 0 and np.mean(cell) > 0.5:
