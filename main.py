...
def configure_from_floorplan(filepath: str, scale: float = None, 
    ...
    if floorplan_type == 'image':
        ...
        print(f"✓ Image size: {width_px}×{height_px} pixels → {width:.1f}×{height:.1f} meters")
    else:
        # For DXF, estimate dimensions
        width = 50.0
        height = 50.0
        print(f"✓ Using estimated dimensions: {width:.1f}×{height:.1f} meters")
        ...
        try:
            ...
            if user_input:
                ...
                if floorplan_type == 'image':
                    ...
                    scale = new_scale
                    print(f"✓ New dimensions: {width:.1f}×{height:.1f} meters")
        except (ValueError, EOFError, KeyboardInterrupt):
            pass
